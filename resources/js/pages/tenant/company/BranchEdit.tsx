...
import { useUpdateBranch } from '@/features/company/hooks/useBranches';
...
export default function BranchEdit({ title, branch }: EditPageProps) {
    ...
    return (
        <TenantLayout>
            ...
            <nav className="mb-5 flex items-center gap-2 text-sm text-neutral-500">
                ...
                <span className="font-medium text-neutral-900">Edit Cabang</span>
            </nav>

            {/* Header */}
            <div className="mb-8">
                <h1 className="text-2xl font-bold tracking-tight text-neutral-900">Edit Cabang</h1>
                <p className="mt-1.5 text-sm text-neutral-500">
                    Perbarui informasi cabang yang sudah terdaftar.
                </p>
            </div>
                ...
                <div className="mb-8 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
                    <div className="bg-gradient-to-r from-primary via-primary-dark to-primary p-6 sm:p-8">
                        <div className="flex items-center gap-5">
                            <div className={cn(
                                ...
                                getAvatarColor(branch.name),
                            )}>
                                {getInitials(branch.name)}
                            </div>
                            <div className="min-w-0 text-white">
                                <h2 className="text-xl font-bold">{branch.name}</h2>
                                <p className="mt-1 text-sm text-white/80">{branch.address || 'Alamat belum diisi'}</p>
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 divide-x divide-neutral-200 border-t border-neutral-200 sm:grid-cols-3">
                        {[
                            ...
                            { label: 'Status', value: branch.is_active ? 'Aktif' : 'Nonaktif' },
                            { label: 'Default', value: branch.is_default ? 'Ya' : 'Tidak' },
                        ].map((item) => (
                            <div key={item.label} className="px-5 py-4">
                                <p className="text-xs font-medium text-neutral-400">{item.label}</p>
                                <p className={cn(
                                    'mt-1 text-sm font-semibold',
                                    item.label === 'Status' && branch.is_active ? 'text-success' : 'text-neutral-900',
                                )}>
                                    {item.value}
                                </p>
                            </div>

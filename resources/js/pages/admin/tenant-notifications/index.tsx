...
function isScheduled(n: AdminTenantNotification) {
...
export default function TenantNotifications({ title }: Props) {
    const [params, setParams] = useState<PaginationParams>({ page: 1, per_page: 15 });
    const { data, isLoading, isError, refetch } = useAdminTenantNotifications(params);
        ...
        <AdminLayout>
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
                <motion.div variants={itemAnim} className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-neutral-900">Notifikasi Tenant</h1>
                        <p className="mt-1 text-sm text-neutral-500">Broadcast notifikasi ke tenant tertentu atau semua tenant.</p>
                    </div>
                    <button
                        onClick={openCreate}
                ...
                <motion.div variants={itemAnim} className="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            value={params.search ?? ''}
                            onChange={(e) => setParams((prev) => ({ ...prev, search: e.target.value || undefined, page: 1 }))}
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Select
                            ...
                            onChange={(v) => setParams((prev) => ({ ...prev, is_active: v || undefined, page: 1 }))}
                            options={[
                            ...
                            {form.target_type === 'specific' && (
                                <div>
                                    ...
                                    <button
                                        ...
                                        className="w-full rounded-xl border border-neutral-300 px-4 py-2.5 text-left text-sm shadow-sm transition-all hover:border-primary focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30"
                                    >
                                    ...
                                    {formErrors.target_tenant_ids && <p className="mt-1 text-xs text-danger">{formErrors.target_tenant_ids}</p>}
                                </div>
                            )}
                ...
                {isLoading ? (
                    ...
                    <motion.div variants={itemAnim} className="flex flex-col items-center gap-4 rounded-xl border border-border bg-white px-6 py-16 text-center shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-sm text-neutral-500">Terjadi kesalahan saat mengambil daftar notifikasi tenant.</p>
                        </div>
                        <button
                            onClick={() => refetch()}
                    ...
                    <motion.div variants={itemAnim} className="flex flex-col items-center gap-4 rounded-xl border border-border bg-white px-6 py-16 text-center shadow-sm">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-sm text-neutral-500">
                                {params.search || params.type || params.is_active
                        ...
                        {(params.search || params.type || params.is_active) && (
                            <button
                                onClick={() => setParams({ page: 1, per_page: params.per_page ?? 15 })}
                                className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-primary-dark"
                            >
                                Reset Filter
                            </button>

...
interface TenantUserFormProps {
    ...
    onSave: (data: TenantUserFormData) => void;
    onCancel?: () => void;
}
...
export default function TenantUserForm({ user, saving, errors = {}, onSave, onCancel }: TenantUserFormProps) {
    const [form, setForm] = useState<TenantUserFormData>({
        name: '',
        email: '',
        password: '',
    ...
    useEffect(() => {
        if (user) {
            setForm({ name: user.name, email: user.email, is_active: user.is_active });
        } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div>
                    <div className="flex items-center gap-2.5 border-b border-neutral-200 pb-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-neutral-500">Buat password yang kuat untuk akun.</p>
                        </div>
                    </div>
                    <div className="mt-5 grid gap-5 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                                            ...
                                            return (
                                                <div key={check.key} className="flex items-center gap-2 text-xs">
                                                    <div className={cn(
                                                        ...
                                                        passed ? 'bg-success text-white' : 'bg-neutral-200 text-neutral-400',
                                                    )}>
                                                        {passed ? (
                                                    ...
                                                    <span className={cn('transition-colors duration-300', passed ? 'text-neutral-700' : 'text-neutral-400')}>
                                                        {check.label}
                                                    </span>
                                                </div>
                                            );
            ...
            <div>
                <div className="flex items-center gap-2.5 border-b border-neutral-200 pb-4">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-neutral-900">Akses</h3>
                        <p className="text-xs text-neutral-500">Tentukan status akses pengguna.</p>
                    </div>
                </div>
                <div className="mt-5">
                    <div className="flex items-center justify-between rounded-xl border border-neutral-200 bg-white px-5 py-4 shadow-sm transition-all duration-200 hover:border-primary/30 hover:shadow-md">
                        <div className="flex items-start gap-3.5">
                            <div className={cn(
                                ...
                                form.is_active ? 'bg-primary text-white shadow-sm' : 'bg-neutral-100 text-neutral-400',
                            )}>
                            ...
                            <div>
                                ...
                                <p className="mt-0.5 text-xs text-neutral-500">
                                    {form.is_active ? 'Pengguna ini aktif.' : 'Pengguna ini tidak aktif.'}
                                </p>
                            </div>
                        </div>

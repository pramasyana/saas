...
export default function AttendanceForm({ attendance, staff, saving, errors = {}, onSave, onCancel }: AttendanceFormProps) {
    ...
    useEffect(() => {
        if (attendance) {
            ...
            setForm({ staff_id: '', date: today, clock_in: '', clock_out: '', status: 'present', notes: '' });
        }
    }, [attendance]);
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
                <div className="flex items-center gap-2.5 border-b border-neutral-200 pb-4">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-neutral-900">Data Absensi</h3>
                        <p className="text-xs text-neutral-500">Catat kehadiran karyawan.</p>
                    </div>
                </div>
                <div className="mt-5 grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        {renderField('Status', 'status', (
                            <Select
                                value={form.status}
                                onChange={(v) => setForm((prev) => ({ ...prev, status: v as AttendanceFormData['status'] }))}
                    ...
                    <div>
                        {renderField('Jam Masuk', 'clock_in', (
                            <input
                                type="time"
                                value={form.clock_in || ''}
                                onChange={(e) => setForm((prev) => ({ ...prev, clock_in: e.target.value }))}
                                className={inputClass('clock_in')}
                            />
                        ))}
                    </div>
                    <div>
                        {renderField('Jam Pulang', 'clock_out', (
                            <input
                                type="time"
                                value={form.clock_out || ''}
                                onChange={(e) => setForm((prev) => ({ ...prev, clock_out: e.target.value }))}
                                className={inputClass('clock_out')}
                            />
                        ))}
                    </div>
                    <div className="sm:col-span-2">
                ...
                <Button type="submit" disabled={saving} className="min-w-[120px]">
                    {saving ? (
                        <span className="inline-flex items-center gap-2">
                            ...
                            {attendance ? 'Menyimpan...' : 'Menambahkan...'}
                        </span>
                    ) : (
                        <span className="inline-flex items-center gap-2">
                            {attendance ? (

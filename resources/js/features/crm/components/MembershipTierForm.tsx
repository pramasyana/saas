...
interface MembershipTierFormProps {
    ...
    onSave: (data: MembershipTierFormData) => void;
    onCancel?: () => void;
}

export default function MembershipTierForm({ tier, saving, errors = {}, onSave, onCancel }: MembershipTierFormProps) {
    const [form, setForm] = useState<MembershipTierFormData>({
        name: '',
        min_points: 0,
        min_total_spent: 0,
        benefits: [],
    ...
    useEffect(() => {
        if (tier) {
            ...
            setForm({ name: '', min_points: 0, min_total_spent: 0, benefits: [], sort_order: 0, is_active: true });
        }
    }, [tier]);
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
                <div className="flex items-center gap-2.5 border-b border-neutral-200 pb-4">
                    ...
                    <div>
                        <h3 className="text-sm font-semibold text-neutral-900">Informasi Tier</h3>
                        <p className="text-xs text-neutral-500">Buat atau edit tier membership.</p>
                    </div>
                </div>
                <div className="mt-5 grid gap-5 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                        {renderField('Nama Tier', 'name', (
                        ...
                        {renderField('Min. Total Belanja', 'min_total_spent', (
                            <div className="relative">
                                ...
                                <input
                                    type="number"
                                    min={0}
                                    value={form.min_total_spent ?? 0}
                                    onChange={(e) => setForm({ ...form, min_total_spent: parseInt(e.target.value) || 0 })}
                                    className={inputClass('min_total_spent', 'pl-10')}
                                />
                            </div>
                        ))}
                    </div>
                    <div>
                        {renderField('Urutan', 'sort_order', (
                            ...
                            />
                        ), 'Semakin kecil semakin rendah')}
                    </div>
                    <div className="sm:col-span-2">
                        {renderField('Benefits', 'benefits', (
            ...
            <div>
                <div className="flex items-center gap-2.5 border-b border-neutral-200 pb-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-neutral-500">Tentukan status tier.</p>
                    </div>
                </div>
                <div className="mt-5">
                        ...
                        <div className="flex items-start gap-3.5">
                            ...
                            <div>
                                ...
                                <p className="mt-0.5 text-xs text-neutral-500">
                                    {form.is_active ? 'Tier ini aktif dan bisa digunakan.' : 'Tier ini tidak aktif.'}
                                </p>
                            </div>
                        </div>

...
function ServiceCard({
    ...
    return (
        <motion.div variants={cardVariants}
            className="group relative flex flex-col rounded-lg overflow-hidden border transition-all duration-300 hover:shadow-xl"
            style={{
                backgroundColor: '#fff',
                borderColor: 'rgba(203,195,215,0.3)',
            }}
        >
            {/* Image placeholder */}
            <div className="h-48 overflow-hidden relative" style={{ backgroundColor: colors.primary + '05' }}>
                <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: colors.primary + '03' }}>
                ...
                <div className="absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold text-white"
                    style={{ backgroundColor: colors.primary + 'E6', backdropFilter: 'blur(8px)' }}
                >
                    FAVORIT
                </div>
            </div>
...
function PackageCard({
    ...
    return (
        <motion.div variants={cardVariants}
            className="group relative flex flex-col rounded-lg overflow-hidden border-2 transition-all duration-300 hover:shadow-xl"
            style={{
                backgroundColor: '#fff',
        ...
        >
            <div className="p-6 flex flex-col gap-3">
                ...
                <h3 className="text-lg font-semibold" style={{ color: colors.text }}>{pkg.name}</h3>
                {pkg.description && (
            ...
            <div className="flex items-center justify-between gap-2 border-t px-6 py-4" style={{ borderColor: colors.primary + '06' }}>
                <div className="flex items-center gap-2">
                    {settings?.show_prices !== false && (
                        <>
                            {showDiscount && originalPrice && (
...
return null;
    ...
    return (
        <section id="services" className="py-16 sm:py-20 lg:py-section-gap-desktop" style={{ backgroundColor: colors.background }}>
                ...
                <FadeIn>
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
                        <div className="space-y-4">
                            {data.title && (
                                <>
                                    <span className="text-xs font-semibold tracking-widest uppercase" style={{ color: colors.primary }}>Ritual Kami</span>
                    ...
                    <div className="flex gap-2 mb-6 overflow-x-auto">
                        {branches!.map((branch) => (
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium whitespace-nowrap transition-all border"
                                style={{
                                    backgroundColor: selectedBranchId === branch.id ? colors.primary : '#fff',
                                    borderColor: selectedBranchId === branch.id ? colors.primary : colors.primary + '20',
                ...
                <div className="min-w-0 flex-1">
                    <AnimatePresence mode="wait">
                        {filtered.length > 0 ? (
                            ...
                            >
                                <div className="inline-flex items-center gap-3 rounded-2xl border px-8 py-5 shadow-sm"
                                    style={{ backgroundColor: 'rgba(255,255,255,0.7)', borderColor: colors.primary + '15' }}
                                >

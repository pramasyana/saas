...
export default function BranchesSection({ data, colors, branches }: Props) {
    ...
    return (
        <section id="branches" className="py-20 sm:py-24 lg:py-32" style={{ backgroundColor: colors.background }}>
            <div className="mx-auto max-w-7xl px-gutter">
                ...
                {items.length > 0 ? (
                                ...
                                >
                                    ...
                                    >
                                        <div className="absolute inset-0 opacity-10">
                                            <div className="absolute top-0 right-0 h-32 w-32 translate-x-8 -translate-y-8 rounded-full bg-white/30" />
                                            <div className="absolute bottom-0 left-0 h-20 w-20 -translate-x-6 translate-y-6 rounded-full bg-white/20" />
                                        </div>
                                        <div className="absolute bottom-0 left-0 right-0 p-6">
                                            <h3 className="text-xl font-black text-white">{b.name}</h3>
                                            {b.is_default && (
                                                <span className="mt-2 inline-block px-3 py-0.5 text-[10px] font-bold uppercase tracking-widest text-white/80 bg-white/20">
                                                    Utama
                                                </span>
                                            )}
                                        </div>
                                    ...
                                    <div className="p-6 space-y-3 text-sm" style={{ color: colors.text_muted, borderTop: `2px solid ${activeCard === b.id ? colors.primary : '#E5E7EB'}` }}>
                                        {b.address && (
                    ...
                    <div className="text-center">
                        <div className="inline-flex items-center gap-3 border px-8 py-5 shadow-sm" style={{ borderColor: colors.primary + '15' }}>
